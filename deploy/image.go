...
func NewImage(ctx *pulumi.Context, name string, args *ImageArgs, opts ...pulumi.ResourceOption) (*Image, error) {
	...
	res.DockerImage, err = docker.NewRegistryImage(ctx, name+"-image", &docker.RegistryImageArgs{
		...
		AuthConfig: &docker.RegistryImageAuthConfigArgs{
			Address:  pulumi.String("docker.io"), // Docker Hub
...
func CommandFromImageInspect(sourceImage string, delimeter string) (*ImageInspect, error) {
	client, err := client.NewClientWithOpts(client.FromEnv, client.WithVersion("1.44"))
	if err != nil {
		return nil, err
	}
...
func NewDockerImage(ctx *pulumi.Context, name string, args *ImageArgs, opts ...pulumi.ResourceOption) (*Image, error) {
	res := &Image{Name: name}
	...
	// Correct registry args
	auth := docker.RegistryArgs{
